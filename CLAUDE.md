...
MCP server bridging Ghidra reverse engineering with AI tools. 231 MCP tools for binary analysis.

- **Package**: `com.xebyte` | **Version**: 5.7.0 | **Java**: 21 LTS | **Ghidra**: 12.0.4
...
| `src/main/java/com/xebyte/core/NamingConventions.java` | Offline (Java) — `NamingConventionsTest` covers function-name verb-tier rules + token-subset duplicate detection + global-name validator (`checkGlobalNameQuality`, `checkGlobalPlateComment`, `isAutoGeneratedGlobalName`). After deploy: `tests/integration/test_safe_write_endpoints.py` + `tests/integration/test_global_endpoints.py` (the latter skips when audit_global isn't registered, i.e., v5.6.x). Also re-run fun-doc benchmark (`--mock --tier fast --compare`). |
...
| `src/main/java/com/xebyte/core/SymbolLabelService.java` — `rename_data` / `rename_global_variable` validator hook | Offline (Java) `NamingConventionsTest` for the rule; `tests/integration/test_global_endpoints.py` for the structured-rejection round-trip. |
